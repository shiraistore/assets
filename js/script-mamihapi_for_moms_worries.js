...
function addCart() {
    $.getJSON('https://cdn.shirai-store.net/assets/json/feature/mamihapi_for_moms_worries_v1_0.json', function (data) {
        //console.log(data);
        ...
        $('.lineup_item_price').each(function () {
            ...
            var price_text;
            var coupon_price_text;
            var result = data.find((u) => u.productUrl === product);
            if (result) {
                //console.log(result);
                var sellingPrice = result.sellingPrice,
                    normalPrice = result.normalPrice,
                    coupon_price = sellingPrice*0.9;

                if (sellingPrice < normalPrice) {
                    price_text = '<span class="mark-sale">SALE</span><span class="normalPrice">¥' + normalPrice.toLocaleString() + '<span class="tax">(税込)</span></span><span class="sellingPrice">¥' + sellingPrice.toLocaleString() + '<span class="tax">(税込)</span></span>';
                } else {
                    price_text = '<span class="sellingPrice">¥' + sellingPrice.toLocaleString() + '<span class="tax">(税込)</span></span>';
                }

                coupon_price_text = '<span class="coupon_price">¥' + coupon_price.toLocaleString() + '<span class="tax">(税込)</span></span>';

                $(this).find('.lineup_item_price_inner').prepend('<div><span class="price_section">price</span>' + price_text + '</div><div><span class="mark-coupon price_section">クーポン利用で10%OFF</span>' + coupon_price_text + '</div>');
            } else {
                // データが存在しなかった時の処理
            }
        });
